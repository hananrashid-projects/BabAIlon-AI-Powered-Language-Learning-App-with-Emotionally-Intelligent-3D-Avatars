...
class RadarChartt extends StatefulWidget {
  final List<int> ticks;
  final List<String> features;
  final List<List<num>> data;
  final List<String> graphNames; // New parameter
  ...
  const RadarChartt({
    ...
    required this.data,
    required this.graphNames, // Accept graph names as an optional parameter
  ...
  factory RadarChartt.light({
    required List<int> ticks,
    required List<String> features,
    required List<List<num>> data,
    required List<String> graphNames,
...
class _RadarChartState extends State<RadarChartt>
  ...
  @override
  void didUpdateWidget(RadarChartt oldWidget) {
...
class RadarChartPainter extends CustomPainter {
  ...
  RadarChartPainter(
    ...
    this.features,
    this.data,
    this.graphNames, // Initialize graphNames
  ...
  void paint(Canvas canvas, Size size) {
    ...
    data.asMap().forEach((index, graph) {
      ...
      if (graphNames != null) {
        final graphName = graphNames![index];

        // Calculate the position to place the graph name and circle at the top-right
        double nameX = size.width -
            230; // Set the starting position to the right (adjust the value as needed)
        double nameY =
            (index * 30) + 20; // Stack names vertically with some spacing

        // Create the TextPainter for the graph name
        ...
        double offsetX = nameX -
            textPainter.width -
            25; // Align the text to the right and leave space for the circle
        double offsetY = nameY; // Use the calculated Y position

        // Draw the graph name
        textPainter.paint(canvas, Offset(offsetX, offsetY));

        // Paint a colored circle next to the graph name (on the right side)
        var circleOffsetX = offsetX +
            textPainter.width +
            10; // Position the circle next to the text
        var circleOffsetY = offsetY +
            textPainter.height / 2; // Align the circle vertically with the text

        // Use the same color as the graph to paint the circle
        var circlePaint = Paint()
          ..color = graphColors[
              index % graphColors.length] // Get the color for the graph
          ..style = PaintingStyle.fill;

        // Draw the circle next to the name
        canvas.drawCircle(Offset(circleOffsetX, circleOffsetY), 7,
            circlePaint); // Draw the circle
      }
    });
  }
